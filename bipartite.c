...
int is_bipartite_BFS(struct graph* graph,struct queque* Q, int s)
{
	int u,v,isbipartite=1;
	struct node* head;
	graph->color[s]=2;
	graph->d[s]=0;
	graph->pred[s]=-1;
...
{	
	...
	int i=0;
	graph=new_graph();
	graph=insert_edge(graph,0,6);
	graph=insert_edge(graph,0,7);
	graph=insert_edge(graph,1,6);
	graph=insert_edge(graph,1,8);
	graph=insert_edge(graph,1,11);
	graph=insert_edge(graph,2,8);
	graph=insert_edge(graph,3,6);
	graph=insert_edge(graph,3,7);
	graph=insert_edge(graph,3,9);
	graph=insert_edge(graph,4,8);
	graph=insert_edge(graph,4,9);
	graph=insert_edge(graph,4,10);
	graph=insert_edge(graph,11,4);
	graph=insert_edge(graph,11,5);
	graph=insert_edge(graph,12,5);
	//the following edge makes the graph non bipartite
	//graph=insert_edge(graph,9,8);
	...
	*/
	...
	if(is_bipartite_BFS(graph,Q,0))
	{	
		printf("graph is bipartite!!!\n");
		//this parition of edges can be done by just doing a single scan through the array
		printf("the set of U edges are:\n");
		for(i=0;i<vertices;i++)
		{	
			//converting float value in d[] to int
			int x=graph->d[i];
			if(x%2==0)
			printf("%d\t",i);
		}
		printf("\n;");
		printf("the set of V edges are:\n");
		for(i=0;i<vertices;i++)
		{
			//converting float value in d[] to int
			int x=graph->d[i];
